...
| 2026-05-16 | **ThemeManager rearchitected as shared block.** `OT_SiteSettings` (_page) → `OT_ThemeManager` (_component) — appears in CMS "Shared content", not the page tree. Added `frontEndDomain` field so multiple sites can each have their own ThemeManager resolved by hostname. Added 7 color override fields in new `OT_Theme` property group (colorBrand, colorBrandHover, colorAccent, colorCanvas, colorSurface, colorCanvasLight, colorSurfaceLight) — empty fields fall back to tokens.css defaults. `getSiteSettings()` now uses raw `client.request()` GraphQL (fetches all instances, filters by domain in JS). `buildThemeCSS()` helper generates inline `:root` + `[data-theme="light"]` overrides. Root `app/layout.tsx` is now async — reads domain from request headers and injects `<style>` tag with token overrides if any colors are set. Old `OT_SiteSettings` stays as orphaned shell in CMS (wastebasket item prevented type deletion). `OT_LandingExperience` removed from codebase — use built-in `BlankExperience` instead. Pushed: 12 content types, 10 display templates, 2 property groups. |
